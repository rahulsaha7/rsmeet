...
const Register = async (nameUSER, email, phone, password) => {
  ...
  try {
    ...
    let text = 'Alternative text will be here' ;
     ...
     let response2 =  Schemas.schemaA.conve.create({userID:id,message:[],pMessage:[],blockList:[],deleteList:[]});
      token = jwt.sign(
        {
          sub: "Auth JWT",
          auth: true,
    ...
    mailSend.sendMailFunc(email,subject,user,Emailpassword,host,port,text,html);
