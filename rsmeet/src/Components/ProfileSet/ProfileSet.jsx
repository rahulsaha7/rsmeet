...
const ProfileSet = ({ regData,setregData}) => {
    ...
    return (
        <>
            ...
            <form className="w-100"  action="" method="POST" encType="multipart/form-data" onSubmit = {(e)=>checkData(e)}>

            <section className="profileImage w-100 d-flex justify-content-center mt-3 p-2">
                <figure  >
                        <img src={regData.dp} alt="BlankDp to visualize user for chooseing Profile Picture"  />
                        <input type="file" hidden name="" id=" " accept=".png,.jpg,.jpeg | image/*" onInput={(event)=>{event.target.setCustomValidity('')}}  ref = {ProfileUploader}  onChange ={(event)=>{uploadHandler(event)} } />
                </figure>

                <figure className="edit" onClick = {()=>ProfileUploader.current.click()} >
                        <span className="d-inline"><MdModeEdit /></span>
                </figure>
            ...
            <span style={{color:"RED",display:err.errDisplay}}> {err.errMessage} </span>

                <section className=" mt-3">
                    <label className="form-label" htmlFor="username">username</label>
                    <div className="usernameHolder p-3 d-flex justify-content-center">

                        <input className="form-control" type="text" name="userName" id="username" placeholder="Please Choose a username" value={regData.username} onChange={(event)=>{setregData({...regData,username:event.target.value})}}  pattern='^[a-zA-Z0-9_@-]+$'  onInvalid={(e) => { e.target.setCustomValidity('only aphnumeric characters with @,_,-  are allowed') }} onInput = {(e)=>{e.target.setCustomValidity('')}} autoComplete="off" required=" " />
                ...
                <section className="w-100 " style={{ textAlign: "right" }}>
                    <button className="py-2 px-3 signInButton mt-3" type="submit"> <span style={{ color: "white" }}> Dashboard  <BsArrowRight color="black" /></span> </button>
